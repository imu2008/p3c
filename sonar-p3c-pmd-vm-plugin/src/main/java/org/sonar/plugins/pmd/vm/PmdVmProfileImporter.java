...
import org.sonar.plugins.pmd.vm.PmdConstants;
import org.sonar.plugins.pmd.vm.PmdLevelUtils;
...
public class PmdVmProfileImporter extends ProfileImporter {

    private final RuleFinder ruleFinder;
    private static final Logger LOG = LoggerFactory.getLogger(PmdVmProfileImporter.class);

    public PmdVmProfileImporter(RuleFinder ruleFinder) {
        super(PmdVmRulesDefinition.REPOSITORY_KEY, PmdConstants.PLUGIN_NAME);
        setSupportedLanguages(Vm.KEY);
      ...
      for (PmdRule pmdRule : pmdRuleset.getPmdRules()) {
        ...
        if (PmdConstants.XPATH_CLASS.equals(ruleClassName)) {
          ...
          if (ruleRef == null) {
            ...
            Rule rule = ruleFinder.find(RuleQuery.create().withRepositoryKey(PmdVmRulesDefinition.REPOSITORY_KEY).withConfigKey(ruleRef));
            if (rule != null) {
