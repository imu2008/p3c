...
public class PmdVmViolationRecorderTest {
  ...
  private PmdVmViolationRecorder pmdViolationRecorder =
    new PmdVmViolationRecorder(fs, ruleFinder, perspectives);
  ...
  public void should_convert_pmd_violation_to_sonar_violation() {
    ...
    File file1 = new File("src/source.vm");
  ...
  private org.sonar.api.rules.Rule createRuleInRuleFinder(String ruleName) {
    ...
    when(ruleFinder.findByKey("pmd-vm", ruleName)).thenReturn(sonarRule);
    when(sonarRule.ruleKey()).thenReturn(RuleKey.of("pmd-vm", ruleName));
    return sonarRule;
  }
