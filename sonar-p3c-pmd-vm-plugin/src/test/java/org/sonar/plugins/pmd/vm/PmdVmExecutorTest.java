...
public class PmdVmExecutorTest {
    PmdVmExecutor pmdExecutor;

    DefaultFileSystem fileSystem = new DefaultFileSystem(new File("."));
    RulesProfile rulesProfile = RulesProfile.create("pmd", "pmd");
    PmdVmProfileExporter pmdProfileExporter = mock(PmdVmProfileExporter.class);
    PmdConfiguration pmdConfiguration = mock(PmdConfiguration.class);
    PmdVmTemplate pmdTemplate = mock(PmdVmTemplate.class);
    JavaResourceLocator javaResourceLocator = mock(JavaResourceLocator.class);
    Settings settings = new Settings();
    PmdVmExecutor realPmdExecutor = new PmdVmExecutor(fileSystem, rulesProfile, pmdProfileExporter,
            pmdConfiguration, javaResourceLocator, settings);
    ...
    public void should_execute_pmd_on_source_files_and_test_files() throws Exception {
        ...
        setupPmdRuleSet(PmdVmRulesDefinition.REPOSITORY_KEY, "simple.xml");
    ...
    public void should_dump_configuration_as_xml() {
        when(pmdProfileExporter.exportProfile(PmdVmRulesDefinition.REPOSITORY_KEY, rulesProfile))
                .thenReturn(PmdTestUtils.getResourceContent("/org/sonar/plugins/pmd/vm/simple.xml"));
    ...
    public void should_dump_ruleset_as_xml() throws Exception {
        ...
        verify(pmdConfiguration).dumpXmlRuleSet(PmdVmRulesDefinition.REPOSITORY_KEY,
                PmdTestUtils.getResourceContent("/org/sonar/plugins/pmd/vm/simple.xml"));
    }

    @Test
    public void should_ignore_empty_test_dir() throws Exception {
        InputFile srcFile = file("src/Class.java", Type.MAIN);
        doReturn(pmdTemplate).when(pmdExecutor).createPmdVmTemplate(any(URLClassLoader.class));
        setupPmdRuleSet(PmdVmRulesDefinition.REPOSITORY_KEY, "simple.xml");
        fileSystem.add(srcFile);

        pmdExecutor.execute();

        verify(pmdTemplate).process(eq(srcFile.file()), any(RuleSets.class),
    ...
    public void should_build_project_classloader_from_javaresourcelocator() throws Exception {
        ...
        pmdExecutor.execute();
        ArgumentCaptor<URLClassLoader> classLoaderArgument =
                ArgumentCaptor.forClass(URLClassLoader.class);
        verify(pmdExecutor).createPmdVmTemplate(classLoaderArgument.capture());
    ...
    public void unknown_pmd_ruleset() throws Exception {
        ...
        when(pmdProfileExporter.exportProfile(PmdVmRulesDefinition.REPOSITORY_KEY, rulesProfile))
                .thenReturn(profileContent);
        when(pmdConfiguration.dumpXmlRuleSet(PmdVmRulesDefinition.REPOSITORY_KEY, profileContent))
                .thenReturn(new File("unknown"));
    ...
    static InputFile file(String path, Type type) {
        return new DefaultInputFile(path).setAbsolutePath(new File(path).getAbsolutePath())
                .setType(type).setLanguage(Vm.KEY);
    }

    private void setupPmdRuleSet(String repositoryKey, String profileFileName) throws IOException {
        File ruleSetDirectory = new File("src/test/resources/org/sonar/plugins/pmd/vm/");
        File file = new File(ruleSetDirectory, profileFileName);
        String profileContent = Files.toString(file, Charsets.UTF_8);
        when(pmdProfileExporter.exportProfile(repositoryKey, rulesProfile))
                .thenReturn(profileContent);
