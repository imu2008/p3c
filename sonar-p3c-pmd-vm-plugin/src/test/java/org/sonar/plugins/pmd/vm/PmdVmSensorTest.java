...
public class PmdVmSensorTest {
  PmdVmSensor pmdSensor;

  Project project = mock(Project.class);
  RulesProfile profile = mock(RulesProfile.class, RETURNS_DEEP_STUBS);
  PmdVmExecutor executor = mock(PmdVmExecutor.class);
  PmdVmViolationRecorder pmdViolationRecorder = mock(PmdVmViolationRecorder.class);
  ...
  public void setUpPmdSensor() {
    pmdSensor = new PmdVmSensor(profile, executor, pmdViolationRecorder, fs);
  }

  @Test
  public void should_execute_on_project_without_main_files() {
    addOneVmFile(Type.TEST);
  ...
  public void should_not_execute_on_project_without_active_rules() {
    ...
    when(profile.getActiveRulesByRepository(PmdVmRulesDefinition.REPOSITORY_KEY).isEmpty()).thenReturn(true);
  ...
  public void should_to_string() {
    ...
    assertThat(toString).isEqualTo("PmdVmSensor");
  }
  ...
  private void addOneVmFile(Type type) {
    File file = new File("x");
    fs.add(new DefaultInputFile(
      file.getName()).setAbsolutePath(file.getAbsolutePath()).setLanguage("vm").setType(type));
  }
}
